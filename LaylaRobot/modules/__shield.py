...
"""
@pbot.on_message(filters.command("antinsfw") & ~filters.edited & ~filters.bot)
async def add_nsfw(client, message):
    ...
    if status == "on" or status == "ON" or status == "enable":
        ...
        await message.reply(" I undestand only `/antinsfw on` or `/antinsfw off` only")
...
__help__ = """
<b> Group Guardian: </b>
✪ Dola can protect your group from NSFW senders, Slag word users and also can force members to use English
