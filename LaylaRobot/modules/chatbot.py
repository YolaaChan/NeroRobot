...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Oda AI Already Activated In This Chat")
            return
        await lel.edit(
            f"Dola AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("Dola AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Dola AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        response = response.replace("Aco", "layla")
        response = response.replace("aco", "layla")
        ...
        response = response.replace("Aco", "layla")
        response = response.replace("aco", "layla")
        response = response.replace("Luna", "layla")
        response = response.replace("luna", "layla")
        pro = response
        if not "en" in lan and not lan == "":
            try:
...
__help__ = """
DOLA AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
